<template>
  <div class="category-page">
    <header class="category-header">
      <div class="container">
        <button class="btn btn-ghost btn-sm" @click="goBack">← Quay lại</button>
        <div>
          <p class="breadcrumb">Truyện / Thể loại</p>
          <h1>{{ pageTitle }}</h1>
          <p v-if="items.length" class="count">{{ items.length }} truyện</p>
        </div>
      </div>
    </header>

    <div v-if="loading" class="loading-text">Đang tải...</div>
    <div v-else-if="error" class="error-text">{{ error }}</div>

    <section v-else class="container category-body">
      <PosterGrid :items="items" />
    </section>
  </div>
</template>

<script setup>
import { ref, onMounted, watch } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import axios from 'axios'
import PosterGrid from '@/components/browse/PosterGrid.vue'
import { mangaApi, MANGA_FEATURED_TAGS } from '@/config/apis'
import { fetchMangaList } from '@/utils/mangaMapper'

const route = useRoute()
const router = useRouter()

const items = ref([])
const pageTitle = ref('')
const loading = ref(true)
const error = ref('')

async function load() {
  loading.value = true
  error.value = ''

  try {
    const tagId = route.params.tagId
    pageTitle.value =
      MANGA_FEATURED_TAGS.find((t) => t.id === tagId)?.label || 'Thể loại truyện'
    items.value = await fetchMangaList(axios, mangaApi.byTag(tagId, 48))
  } catch {
    error.value = 'Không thể tải danh mục.'
  } finally {
    loading.value = false
  }
}

function goBack() {
  router.push('/truyen')
}

watch(() => route.params.tagId, load)
onMounted(load)
</script>

<style scoped>
.category-page {
  padding-bottom: 48px;
}

.category-header {
  padding: 20px 0;
  border-bottom: 1px solid var(--border);
  margin-bottom: 24px;
}

.breadcrumb {
  font-size: 0.75rem;
  color: var(--text-muted);
  margin: 12px 0 4px;
  text-transform: uppercase;
  letter-spacing: 0.06em;
}

.category-header h1 {
  font-size: clamp(1.5rem, 3vw, 2rem);
  margin: 0;
  font-weight: 800;
}

.count {
  color: var(--text-muted);
  font-size: 0.875rem;
  margin: 6px 0 0;
}

.category-body {
  padding-bottom: 8px;
}
</style>
