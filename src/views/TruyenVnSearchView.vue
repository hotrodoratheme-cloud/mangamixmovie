<template>
  <div class="browse-page">
    ...
    <section v-if="showSearchResults" class="container results-section">
      ...
      <LoadingSkeleton v-if="loading" variant="search" card-type="manga" :count="8" />
    ...
    <template v-else>
      ...
      <LoadingSkeleton v-if="homeLoading" variant="browse-home" card-type="manga" :count="8" />
      <div v-else-if="homeError" class="container error-text">{{ homeError }}</div>

      <div v-else class="container home-sections">
        <ContinueSection
          scope="manga_vn"
          title="Tiếp tục đọc truyện VN"
...
<script setup>
defineOptions({ name: 'TruyenVnSearchView' })
