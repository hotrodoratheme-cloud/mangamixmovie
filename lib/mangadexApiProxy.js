import { getMangadexCacheControl, PROXY_CACHE_NO_STORE } from './proxyCache.js'
...
export async function handleMangadexApiRequest(req, res) {
  ...
  try {
    ...
    res.setHeader('Cache-Control', getMangadexCacheControl(target, status))
    res.end(body)
  } catch (err) {
    res.statusCode = 502
    res.setHeader('Access-Control-Allow-Origin', '*')
    res.setHeader('Content-Type', 'application/json')
    res.setHeader('Cache-Control', PROXY_CACHE_NO_STORE)
