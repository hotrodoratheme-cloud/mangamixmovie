...
const CHAPTER_CDN_HOSTS = ['sv1', 'sv2', 'sv3', 'sv4']

export function buildOtruyenChapterUrl(chapterId, host = 'sv1') {
  const id = String(chapterId || '').trim()
  if (!id) throw new Error('Missing chapter id')
  return `https://${host}.otruyencdn.com/v1/api/chapter/${id}`
}
...
export function isAllowedOtruyenChapterUrl(url) {
  try {
    ...
    return (
      (hostname.endsWith('otruyencdn.com') || hostname.endsWith('otruyencdn.net')) &&
...
function buildCandidateUrls(chapterId, sourceUrl) {
  ...
  if (chapterId) {
    for (const host of CHAPTER_CDN_HOSTS) {
      add(buildOtruyenChapterUrl(chapterId, host))
    }
  }
